...
write.csv(prediction_df, "~/toolkit/learning_projects/titanic/titanic_prediction.csv", row.names = FALSE)

```

[Update] After submitting the results, the success rate turned out to be 77.99%, which is... actually not terrible; the error rate difference is 6%, which intuitively seems okay as far as robustness goes. In terms of performance on the leaderboards, it's within the top quartile, which again is not bad for such a simple approach.