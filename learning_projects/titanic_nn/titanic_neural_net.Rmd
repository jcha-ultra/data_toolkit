...
make_testing_output <- function(model, testing_data_final, write_name = NULL, dir = "~/repos/toolkit/learning_projects/titanic_nn") {
  titanic_testing_output <- predict(model, testing_data_final) %>%
    compile_titanic_output_1(testing_data_final, rep(NA, nrow(testing_data_final)), nn_output = .) %>% mutate(prediction = round(nn_output))
